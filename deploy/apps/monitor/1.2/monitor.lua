...
function app.on_open(w, h)
  ...
  local ch_h = narrow and 58 or 70
  local AX = 38                                        -- gutter for the dBm axis labels
  chart = ui.chart(w - AX - 8, ch_h, 72, C.accent, C.sub)   -- series 1 = RSSI, 2 = noise
  chart:pos(AX, 18)
  chart:range(RSSI_MIN, RSSI_MAX)
  chart:axis(3, AX)                                    -- dBm scale down the left
  ...
  local fy = my + (narrow and 50 or 36)
  ...
  for i = 1, rows do
    feed_rows[i] = ui.label("", 4, fy + 16 + (i - 1) * 15, 12, C.text)
    feed_rows[i]:width(w - 8)
  end
