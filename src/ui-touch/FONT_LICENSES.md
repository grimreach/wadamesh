...
The exact source releases, SHA-256 hashes, glyph ranges, converter version,
and output procedure are pinned in `scripts/build/gen-touch-fonts.sh`.

## Regenerating the assets

The generator requires Bash, curl, unzip, Node.js/npx, Perl, and either
`shasum` or `sha256sum`. From the repository root, run:

```sh
scripts/build/gen-touch-fonts.sh
```

To verify that the checked-in C files match a clean regeneration without
changing them, run:

```sh
scripts/build/gen-touch-fonts.sh --check
```
